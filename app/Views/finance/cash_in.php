...
<div class="page-header">
    <div>
        <p class="page-title">Kas Masuk</p>
        <p class="page-subtitle">Pemasukan kas di luar penjualan (mis. setoran modal, pelunasan lain).</p>
    </div>
</div>
...
<div class="chart-grid" style="grid-template-columns:1fr 1.6fr;">
    ...
    <div class="card-surface card-pad">
        <p class="section-card-title">Catat Kas Masuk</p>
        <form method="POST" action="/finance/cash-in" data-loading-text="Menyimpan...">
            <?= csrf_field() ?>
            <div class="form-group">
                <label class="form-label">Kategori<span class="required">*</span></label>
                <input type="text" name="category" class="form-control" placeholder="Contoh: Setoran Modal" required>
            </div>
            <div class="form-group">
    ...
    <div class="card-surface card-pad">
        <p class="section-card-title">Riwayat Kas Masuk</p>
        <?php if (empty($rows)): ?>
            <div class="empty-state" style="padding:24px 8px;"><p>Belum ada catatan kas masuk manual.</p></div>
        <?php else: ?>
